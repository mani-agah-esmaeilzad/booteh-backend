// File: src/pages/WlbAssessment.tsx
...
const WlbAssessment = () => {
    ...
    const scenarioInfo = {
        category: "ارزیابی تعادل کار و زندگی",
        context: "در این سناریو، شما با دکتر علوی، یک روانشناس سازمانی، در مورد دیدگاهتان نسبت به کار و زندگی شخصی گفتگو خواهید کرد.",
        characters: [{ name: 'دکتر علوی', role: 'روانشناس سازمانی', avatar: '🧑‍🏫' }]
    };
    ...
    const handleTimeUp = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch('/api/assessment/finish-wlb', {
                method: 'POST',
            ...
            localStorage.setItem('wlb_results', JSON.stringify(data.analysis));
            router.push('/results');
        } catch (error) {
    ...
    useEffect(() => {
        const startSession = async () => {
            ...
            try {
                const token = localStorage.getItem('token');
                const response = await fetch('/api/assessment/start-wlb', {
                    method: 'POST',
    ...
    const handleSendMessage = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch('/api/assessment/chat-wlb', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
            ...
            if (data.reply) {
                const aiMessage: ChatMessage = { type: 'ai', content: data.reply, character: 'دکتر علوی' };
        ...
        <div className="min-h-screen flex flex-col bg-gray-50 rtl">
          ...
          <Dialog open={isIntroModalOpen && !loading}>
            <DialogContent className="sm:max-w-md bg-white/90 backdrop-blur-lg border-gray-200 rounded-2xl shadow-luxury">
              ...
              <div className="py-4 space-y-6">
                <div>
                  <h3 className="font-semibold text-center text-lg mb-4 text-gray-800">شخصیت حاضر در جلسه</h3>
          ...
          <header className="bg-white border-b p-4 sticky top-0 z-10">
            <div className="relative max-w-3xl mx-auto flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-lg font-bold text-gray-800">ارزیابی تعادل کار و زندگی</h1>
              </div>
          ...
          <main className="flex-1 overflow-y-auto p-4">
            <div className="max-w-3xl mx-auto space-y-4">
              ...
              {isTyping && <div className="text-sm text-gray-500 pr-12 text-right">دکتر علوی در حال تایپ...</div>}
