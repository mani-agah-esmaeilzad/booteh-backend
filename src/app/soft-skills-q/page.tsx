// src/app/soft-skills-q/page.tsx
...
export default function SoftSkillsQuestionnairePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ✅ آدرس API به نام جدید تغییر کرد
      const response = await fetch('/api/soft-skills-q', {
        method: 'POST',
        headers: { 
      ...
      if (response.ok) {
        toast.success("پرسشنامه با موفقیت ثبت شد. به صفحه اصلی منتقل می‌شوید.");
        router.push('/');
      } else {
    ...
    <div dir="rtl" className="min-h-screen bg-gray-50 flex items-center justify-center p-4 sm:p-6 lg:p-8">
      ...
      <div className="max-w-4xl w-full bg-white p-8 rounded-2xl shadow-lg border border-gray-200">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 mt-2 max-w-2xl mx-auto">قبل از شروع شبیه‌سازی‌های تعاملی، لطفاً به مهارت‌های خود امتیاز دهید.</p>
        </div>
