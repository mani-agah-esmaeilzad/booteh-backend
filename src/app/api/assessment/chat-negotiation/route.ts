...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const systemPrompt = NEGOTIATION_SYSTEM_PROMPT.split('# Conversation Flow and Rules')[0].replace('{user_name}', userName);
    ...
    await connection.execute(
        'INSERT INTO chat_messages (assessment_id, user_id, message_type, content, character_name) VALUES (?, ?, ?, ?, ?)',
        [assessmentId, userId, 'ai', aiReply, 'آقای توکلی']
    );

    return NextResponse.json({ success: true, reply: aiReply });

  } catch (error) {
    console.error('Error in Negotiation chat:', error);
