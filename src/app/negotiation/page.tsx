...
const NegotiationAssessmentPage = () => {
    ...
    const scenarioInfo = {
        category: "ارزیابی مهارت‌های مذاکره",
        context: "در این سناریو، شما با آقای توکلی، مدیر خرید، برای فروش محصولات خود مذاکره خواهید کرد.",
        characters: [{ name: 'آقای توکلی', role: 'مدیر خرید', avatar: '👔' }]
    };
    ...
    const handleTimeUp = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch('/api/assessment/finish-negotiation', {
                method: 'POST',
            ...
            localStorage.setItem('negotiation_results', JSON.stringify(data.analysis));
            router.push('/results');
        } catch (error) {
    ...
    useEffect(() => {
        const startSession = async () => {
            ...
            try {
                const token = localStorage.getItem('token');
                const response = await fetch('/api/assessment/start-negotiation', {
                    method: 'POST',
    ...
    const handleSendMessage = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch('/api/assessment/chat-negotiation', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
            ...
            if (data.reply) {
                const aiMessage: ChatMessage = { type: 'ai', content: data.reply, character: 'آقای توکلی' };
        ...
        <div className="min-h-screen flex flex-col bg-gray-50 rtl">
          ...
          <Dialog open={isIntroModalOpen && !loading}>
            <DialogContent className="sm:max-w-md bg-white/90 backdrop-blur-lg border-gray-200 rounded-2xl shadow-luxury">
              ...
              <DialogFooter>
                <Button onClick={() => { setIsIntroModalOpen(false); setAssessmentStarted(true); }} className="w-full bg-indigo-600 text-white hover:bg-indigo-700 text-lg py-6">
                  شروع گفتگو
                </Button>
              </DialogFooter>
            </DialogContent>
          ...
          <header className="bg-white border-b p-4 sticky top-0 z-10">
            <div className="relative max-w-3xl mx-auto flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-lg font-bold text-gray-800">ارزیابی مهارت‌های مذاکره</h1>
              </div>
          ...
          <main className="flex-1 overflow-y-auto p-4">
            <div className="max-w-3xl mx-auto space-y-4">
              ...
              {isTyping && <div className="text-sm text-gray-500 pr-12 text-right">آقای توکلی در حال تایپ...</div>}
...
export default NegotiationAssessmentPage;
